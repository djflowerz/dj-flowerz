...
export class UserComponent implements OnInit {
  ...
  findAllUsers(){
    this.api.findAllUsers().subscribe((data: Customer[]) => {
      console.log(data);
  ...
  initUsers(data: Customer[]) {
    ...
    const dtOptions = {
      ...
      columns: [
        ...
        },
        {
          ...
          defaultContent: '<i style="color: blue; cursor: pointer; align-content: center; text-align: center" class="fa fa-trash"></>',
          responsivePriority: 1
        },
      ],
      columnDefs: [{
    ...
    $(listingDiv).on('click', 'td.delete', function() {
      // @ts-ignore
      const tr = $(this).closest('tr');
      const row = scope.listingDatatable.row(tr);
      const data = row.data();
      ...
      scope.open(scope.deleteUserRef, 'md');
    });
  }
